...
cvevents <- function(tbl, when, what, where, details = NULL) {
  ...
  for (i in seq_len(nrow(tbl))) {
    block <- '<div style="display: flex; justify-content: space-between; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5em;">\n'
    
    # Left side: what + where + optional details
    block <- paste0(block, '  <div style="flex: 1 1 70%; min-width: 250px;">\n')
    ...
    if (!is.null(details)) {
      ...
      if (length(detail_items) > 0) {
        block <- paste0(block, '    <ul style="margin: 0.5em 0 0 1em; padding-left: 0;">\n')
        for (item in detail_items) {
          # Auto-link any URLs
          item <- gsub("(https?://[^\\s)]+)", "<a href='\\1' target='_blank'>\\1</a>", item, perl = TRUE)
    ...
    block <- paste0(
      block,
      '  <div style="flex: 0 0 25%; text-align: right; font-weight: bold; min-width: 120px;">',
      tbl[[when]][i],
      '</div>\n'
    )
...
:::{.pdf .no-print}
Download the [PDF](docs/cv-pdf/cv-pdf.pdf) version.
</br>
