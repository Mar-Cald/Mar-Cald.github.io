...
The prior (grey) represents our initial guess about plausible values;
after observing data, it updates to the posterior (blue), which narrows
around the most likely value.
...
Suppose we want to estimate the Stroop effect (0.060–0.120s). We can
adopt priors centered at zero with varying standard deviations (τ = 0.1,
0.25, 0.5) to reflect increasing skepticism.
...
parse = TRUE
  ...
  # prior/posterior lines as before
  geom_line(aes(color = dist, linetype = dist), linewidth = 1.2) +
  #geom_vline(xintercept = 0, color = "black", linewidth = 0.8) +
  scale_fill_manual(
    values = c(main = "#2C7BB6",  # dark blue (Pd side)
               tail = "#B3D7F4"), # light blue (small opposite tail)
    guide = "none"
  ) +
...
> $P(ver > 0 \text{ AND } vis > 0) \leq P(ver > 0 \text{ OR } vis > 0)$
...
> What are the main barriers to correct statistical understanding and
> practice in your field? What would help us improve?
